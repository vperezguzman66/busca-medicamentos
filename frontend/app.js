...
async function loadStores() {
  try {
    ...
    STORES = [
      { id: "salcobrand", name: "SalcoBrand", configured: true },
      { id: "ahumada", name: "Farmacias Ahumada", configured: true },
      { id: "drsimi", name: "Doctor Simi", configured: true },
    ];
  }
...
const HISTORY_KEY = "buscamedicamentos_history";
...
function badgeClass(storeId) {
  return { salcobrand: "badge-salcobrand", ahumada: "badge-ahumada", drsimi: "badge-drsimi", cruzverde: "badge-cruzverde" }[storeId] || "";
}
...
function renderBatchResults(data) {
  ...
  let html = `<div class="results-header">
    <h2>${total_queries} medicamento${total_queries !== 1 ? "s" : ""} buscado${total_queries !== 1 ? "s" : ""}</h2>
...
searchForm.addEventListener("submit", async e => {
  ...
  if (!stores) { alert("Selecciona al menos una farmacia."); return; }
