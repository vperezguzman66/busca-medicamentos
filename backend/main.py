...
from scrapers import AhumadaScraper, CruzVerdeScraper, DrSimiScraper, SalcoBrandScraper
...
_cruzverde_scraper = CruzVerdeScraper()

SCRAPERS = {
    "salcobrand": SalcoBrandScraper(),
    "ahumada": AhumadaScraper(),
    "drsimi": DrSimiScraper(),
    "cruzverde": _cruzverde_scraper,
}

# ── App lifecycle — shared Playwright browser for Cruz Verde ─────────────────
...
async def lifespan(app: FastAPI):
    ...
    _cruzverde_scraper.browser = browser
    yield
    await browser.close()
    await playwright.stop()


app = FastAPI(title="BuscaMedicamentos API", lifespan=lifespan)
...
async def list_stores():
    return [
        {"id": "salcobrand", "name": "SalcoBrand", "url": "https://salcobrand.cl", "configured": True},
        {"id": "ahumada", "name": "Farmacias Ahumada", "url": "https://www.farmaciasahumada.cl", "configured": True},
        {"id": "drsimi", "name": "Doctor Simi", "url": "https://www.drsimi.cl", "configured": True},
        {"id": "cruzverde", "name": "Cruz Verde", "url": "https://www.cruzverde.cl", "configured": True},
    ]
...
async def search_batch(
    ...
    stores: str = Form(default="salcobrand,ahumada,drsimi"),
    ...
    queries = [q for q in queries if not q.lower().startswith("producto") and not q.lower().startswith("medicamento")]
    queries = queries[:30]

    if not queries:
        raise HTTPException(status_code=400, detail="El archivo CSV no contiene medicamentos")
